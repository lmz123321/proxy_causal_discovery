# Add background knowledge (bk) py-causal-learn:
# - init a bk object and define required/forbbiden edges by three ways:
#   1. add_by_node: input GraphNode object
#   2. add_by_pattern: input nodes' names, A,B and forbid A->B
#   3. add_by_tier: input tier for nodes, forbid A->B if tier(A)>=tier(B)
# - the skelelon discovery and orientation alg. will ask the input backgroundknowledge object
# - whether an edge is required/forbidden by is_required() or is_forbidden() function.

# for the AD problem, we want to add the following knowledges:
# - sex,edu,apoe can not be pointed to
# - Y can not be pointed to by BRegions
# - hippocampus can not be pointed to by other BRegions
# so, we can assign:
# - attributes  tier=1
# - Y           tier=2
# - Hippocampus tier=3
# - BRegions    tier=4
# and change the bk class into a CustomBK class, where we
#   1. forbid A->B if tier(A)>tier(B)
#   2. forbid A->B if tier(A)=tier(B)!=highest tier (to allow edges among BRegions)

import re
from typing import Set, Tuple, Dict

from timediscover.graph.Node import Node


class CustomBackgroundKnowledge(object):
    def __init__(self):
    ...
    def is_forbidden(self, node1: Node, node2: Node) -> bool:
        ...
        if self.tier_value_map.keys().__contains__(node1) and self.tier_value_map.keys().__contains__(node2):
            ...
            if self.tier_value_map.get(node1) > self.tier_value_map.get(node2):
                return True
            if self.tier_value_map.get(node1) == self.tier_value_map.get(node2) and self.tier_value_map.get(node1) != max(self.tier_map.keys()):
                return True
